;;; init-auto-save.el --- 自动保存文件               -*- lexical-binding: t; -*-

;; Author: ManateeLazyCat@jianshu.com
;; Keywords: lisp, tools
...
(defgroup local-auto-save nil
  "Auto save file when Emacs idle."
  :group 'local-auto-save)
...
(defun local-auto-save-start ()
  "启动自动保存."
  (interactive)
  ;; run-with-idle-timer 函数的意思就是在 auto-save-idle 定义的描述以后自动执行 auto-save-buffers 函数
  ;; #' 的意思就是在 runtime 执行的时候再展开 auto-save-buffers 函数
  (run-with-idle-timer local-auto-save-idle t #'local-auto-save-buffers)
  )

;; 让程序在设置之后能够自动启动
(add-hook 'after-init-hook (lambda ()
			     (if local-auto-save-enable
				 (local-auto-save-start))))
